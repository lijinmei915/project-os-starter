...
Workbench UI 的左侧工作区按“流程、知识、执行、资产、Agent 配置”分层，而不是按内部实现模块分层。

原则：

- `项目流程` 放从理解项目到复盘沉淀的生命周期阶段。
- `知识记忆` 放项目上下文、用户偏好、团队规则、决策记录和会话摘要。
- `任务执行` 放对话、计划、待办、运行记录和执行结果。
- `工程资产` 放文件、文档、脚本、模板和 schema。
- `Agent 配置` 放模型、技能、工具、权限和适配器。

这样分的原因：

- 流程入口回答“现在项目推进到哪一步”。
- 知识入口回答“系统应该记住什么上下文”。
- 执行入口回答“当前任务怎么计划、运行和回看”。
- 资产入口回答“项目里有哪些可检查、可交付的文件”。
- 配置入口回答“Agent 和桌面端本身怎么连接和运行”。

不要把资源入口混进项目流程。例如 `Agent 配置` 和 `工程资产` 是常驻能力，不是项目生命周期阶段；项目流程里只保留 `认识项目`、`定义目标`、`工作规则`、`设计实现`、`验证交付`、`复盘沉淀`。

`Skills` 属于 `Agent 配置`，因为它们是 Agent 能力扩展；`工程资产` 只负责查看和维护文档、代码、schema、脚本和模板这类项目资产。

`系统设置` 不属于左侧工作区树。顶部右侧保留轻量 `主题` 入口，并在其右侧提供独立齿轮入口承载项目 registry、provider、本地存储、安全隐私和高级设置，避免把应用级配置混进当前项目工作内容。
