...
覆盖截图：新项目状态识别、推荐补齐方案、老项目代码来源、当前工作目录、体检结果。
...
| Variant | 用途 | 示例 |
|---------|------|------|
| `with-options` | 标题下方是选择卡片组 | 代码来源、项目状态识别 |
| `with-form` | 标题下方是输入框、上传区或按钮 | 当前工作目录、Git 仓库地址 |
| `with-results` | 标题下方是结果、资料或体检列表 | 推荐补齐方案、体检结果 |
...
用途：展示“推荐补齐方案”里的必选入口文件和推荐工程文件，让文档卡片在桌面保持稳定三列节奏，小屏再收缩到一列。
...
用途：在推荐补齐方案区顶部，用一行摘要告诉用户当前推荐结果来自哪些状态信号和下一步动作，而不是让用户在左侧说明和右侧配置之间来回对照。
...
- `skill` 是 Q2 下一步动作之一，默认只生成最小 Agent Skill 骨架，不展示二级模板选择。
- Agent Skill 补齐参考 OpenDesign 的证据推导思路：先以 `SKILL.md` 为行为真源，后续根据目标产物、已有文件、下一步动作和验收要求再补 `references/`、`assets/`、`scripts/`、`schemas/`、`fixtures/`、`tests/` 或分发文件。
- `skill` 用于沉淀可复用 agent skill；它不等同于 Q3 的 AI/RAG Runtime 扩展。
...
- 用户界面只展示：系统理解了什么、建议下一步做什么、为什么建议、需要用户确认什么。
- `INSTALL` / `INIT` / `AUDIT` / `HYBRID` 等内部路由名、回归测试范围、模板同步结果和开发过程说明，不作为普通用户文案展示。
- 内部验证信息保留在测试输出、`HANDOFF.md` 或开发记录中；只有验证失败会影响用户下一步时，才转换成可行动的风险提示。
- 不展示“接下来跑完整回归”“确认没有破坏原行为”等开发者过程叙述。
