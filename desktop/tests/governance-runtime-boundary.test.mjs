...
test("keeps retired governance tooling out of the Desktop Runtime", () => {
  ...
  const preview = fs.readFileSync(path.join(root, "desktop/vite.config.js"), "utf8");
  ...
  assert.doesNotMatch(preview, /scripts\/check-runtime\.sh|governance:runtime/);
  assert.doesNotMatch(app, /lower\.starts_with\("(?:scripts|adapters|templates)\/"\)/);
});
