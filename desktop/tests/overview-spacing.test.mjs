...
test("overview pages use the shared semantic section spacing token", () => {
  ...
  assert.match(themeStyles, /--desktop-space-overview-section: var\(--space-20\)/);
  assert.match(workspaceStyles, /\.overviewSurface,\s*\.projectOverviewSurface\s*\{\s*display: grid;\s*gap: var\(--desktop-space-overview-section\)/);
