...
const governanceSpecs = {
  ...
  "validation-checks": { title: "检查项", description: "定义项目需要验证什么，以及检查由哪条受控链路执行。", sources: ["docs/TESTING.md", "scripts/check-runtime.sh", "scripts/check-ai-project.sh"], meta: "测试策略或检查入口变化时更新", sections: [["检查目录", "按风险选择验证类型", <OverviewTagList items={["Runtime 健康", "行为路由", "设计一致性", "代码测试", "回归与交付"]} />], ["执行边界", "检查从受控入口进入", "检查项在任务执行或受控检查链路中运行；本页只说明可验证范围，不执行命令，也不展示终端输出。"], ["覆盖范围", "验证代码、规则和可分发内容", "UI 改动关注 token、交互和浏览器验收；路由、契约与模板改动分别运行对应测试、构建和同步检查。"]] },
