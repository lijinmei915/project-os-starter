...
const memorySpecs = {
  "project-facts": { title: "项目事实", description: "集中查看已确认的项目事实、来源与新鲜度。", sources: [".omnidesk/data/project-profile.json", ".omnidesk/cache/workspace-facts.json"], sections: [["事实档案", "项目定位、阶段和技术组成只以已确认事实呈现。"], ["来源依据", "每项事实保留来源文件，推断内容不直接写成项目事实。"], ["新鲜度", "事实变化后需要重新扫描或确认，避免使用过期上下文。"]] },
  "user-preferences": { title: "用户偏好", description: "区分当前项目协作偏好与跨项目长期偏好。", sources: [".omnidesk/data/project-profile.json"], sections: [["项目偏好", "记录当前项目的表达方式、协作边界和风险取向。"], ["全局偏好", "跨项目长期有效的偏好不混入当前项目事实。"], ["确认边界", "从对话提取的偏好需要用户确认后才作为稳定偏好使用。"]] },
  "long-term-memory": { title: "长期记忆", description: "沉淀可跨轮次复用的决策、经验和重要上下文。", sources: [".omnidesk/data/memory/", "docs/DECISIONS.md", "docs/LESSONS.md"], sections: [["重要决策", "长期有效的取舍归档为决策，不复制当前任务细节。"], ["经验约束", "已经验证的避坑规则沉淀为经验教训。"], ["保留策略", "只有后续工作仍需要的内容进入长期记忆，临时对话不默认沉淀。"]] },
  "conversation-summary": { title: "会话摘要", description: "把对话中的主题、结论和未决事项压缩为可继续的上下文。", sources: [".omnidesk/data/conversations/", ".omnidesk/data/tasks/"], sections: [["会话主题", "保留当前讨论的核心主题，短追问可以继承上下文。"], ["已得结论", "已确认的结论、约束和执行结果被归纳，避免重复解释。"], ["未决事项", "尚待确认或尚未执行的事项保留为后续入口，不直接当成已完成。"]] },
};
...
const governanceSpecs = {
  "collaboration-boundary": { title: "协作边界", description: "明确用户、Agent 与受控系统各自负责什么。", sources: ["AGENTS.md"], meta: "规则随协作方式和路由变化更新", sections: [["用户负责", "决定方向和承担最终确认", "确认目标、优先级、范围取舍，以及涉及写入、发布或高风险操作的最终决定。"], ["Agent 负责", "在明确范围内推进工作", "读取项目上下文、梳理影响、实现已确认改动、运行对应检查，并同步必要的交接信息。"], ["协作原则", "当前请求优先，保持范围可控", "先处理当前明确事项；不擅自扩展功能，不把内部流程负担转嫁给用户，也不重复询问已有上下文。"]] },
  "execution-permissions": { title: "执行权限", description: "明确哪些操作可以直接进行，哪些必须由你确认。", sources: ["AGENTS.md"], meta: "规则随权限边界和执行方式变化更新", sections: [["可直接进行", "不改变项目外部状态的常规工作", "读取项目文件、分析现状、运行已有检查，以及在你明确要求的范围内完成常规实现和验证。"], ["需要确认", "会改变内容、范围或外部状态的操作", "删除或覆盖文件、发布或推送、批量重构、执行有副作用命令，以及需求不清时的方向选择，都要先征得你的确认。"], ["受控执行", "命令和写入都应有明确边界", "检查和任务执行通过已有白名单与确认链路完成；终端、Patch 和应用结果分别归任务执行的对应页面管理。"]] },
  "documentation-rules": { title: "文档规则", description: "让每类信息只写在它真正负责的文档里。", sources: ["docs/DOCUMENTATION.md", "docs/NAMING.md"], meta: "规则随文档职责和命名边界变化更新", sections: [["文档归属", "按问题选择唯一的事实来源", "AI 行为归 AGENTS.md，当前状态归 PROJECT.md，交接归 HANDOFF.md，长期工程规则归 docs/ 下对应专题文档。"], ["更新规则", "改到职责变化时才同步", "已有事实来源时只引用或摘要；新增或改变 docs 职责时登记文档结构清单并运行对应检查。"], ["运行证据", "状态与证据不混入产品文档", "运行时状态由 .omnidesk 的分区保存，真实评测与检查证据按 Runtime 策略保留；Markdown 只解释边界与入口。"]] },
  "system-architecture": { title: "系统架构", description: "说明 OmniDesk 从工作台到本地 Runtime 的分层和运行边界。", sources: ["docs/ARCHITECTURE.md"], meta: "架构职责变化时更新", sections: [["系统分层", "从工作台到本地 Runtime", <OverviewTagList items={["Workbench UI", "Runtime contract", "Domain services", "Repository", "Local project"]} />], ["唯一产品入口", "Desktop Runtime 承担执行语义", "Tauri Desktop 通过受控 Runtime 执行；浏览器 Preview 只读，不提供隐藏写入、终端或检查入口。"], ["运行边界", "界面、Runtime 与状态分工", "工作台呈现并发起受控请求；Runtime 持有权限、事务和工具边界；.omnidesk 保存状态与证据。"]] },
  "data-contracts": { title: "数据契约", description: "定义 Runtime 状态、工作面数据与写入校验可以依赖的稳定结构。", sources: ["schemas/project-state.schema.json", "desktop/src-tauri/src/runtime/"], meta: "Schema 或 Runtime 数据语义变化时更新", sections: [["状态与事实", "用户数据、运行事件、缓存与证据分区", <OverviewTagList mono items={["data", "runtime", "cache", "evidence"]} />], ["工作面契约", "事实如何变成稳定页面", "工作台从 Runtime 投影读取状态，不在前端复制业务状态机或直接写入工程。"], ["校验与写入", "结构变化必须经过受控通道", "Repository、schema 与事务负责校验；工程写入与检查继续通过独立审批。"]] },
  "code-structure": { title: "实现结构", description: "说明用户界面、本地 Runtime 与证据如何分工。", sources: ["docs/ARCHITECTURE.md", "desktop/src/main.jsx", "desktop/src-tauri/src/main.rs"], meta: "目录职责或模块边界变化时更新", sections: [["工作台界面", "桌面端页面、组件和呈现逻辑", "desktop/src 负责 React 工作台、路由登记和受控请求投影；页面不直接读写工程文件。"], ["本地 Runtime", "设备上的项目读取、持久化与受控工具", "desktop/src-tauri/src/runtime 负责工作区、任务、Agent Run、Provider、Patch、检查和恢复。"], ["证据与测试", "以可验证结果约束发布", "desktop/evals 保存基线契约，.omnidesk/evidence 保存真实 trace；常规回归不依赖旧 CLI 或分发脚本。"]] },
