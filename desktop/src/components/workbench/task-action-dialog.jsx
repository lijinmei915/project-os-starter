...
export function TaskActionDialog({
  ...
  React.useEffect(() => {
    setIsolate(task?.executionMode === "isolated");
    ...
    <Dialog open={Boolean(action)} onOpenChange={(open) => { if (!open) onClose?.(); }}>
      <DialogContent className="taskActionDialogContent" description={task?.title || "未命名任务"} title={title}>
        ...
        <div className="taskCreateActions">
          ...
          {["review", "start"].includes(mode) ? <Button disabled={!modelAvailable || modelChecking} title={modelAvailable ? "确认并开始执行" : "请先测试当前模型可用性"} type="button" variant="primary" onClick={() => onConfirmStart?.({ isolate })}>{modelChecking ? "正在检测模型" : "确认并开始"}</Button> : null}
