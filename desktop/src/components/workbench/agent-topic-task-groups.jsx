            ...
            {group.tasks.map((task) => (
              <article className={`agentTaskQueueItem agentTaskListButton status-${task.status || "unknown"}${task.id === currentTaskId ? " active" : ""}`} key={task.id} onClick={(event) => openTaskFromCard(event, task.id)}>
                <div className="agentTaskQueueMain">
                  <div className="taskCardStatusLine"><Badge status={taskStatusLabel(task)}>{taskStatusLabel(task)}</Badge><time>{taskUpdatedLabel(task)}</time><DropdownMenu><DropdownMenuTrigger asChild><button className="sectionIconAction" type="button" onPointerDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()} aria-label="任务更多操作"><MoreVertical aria-hidden="true" /></button></DropdownMenuTrigger><DropdownMenuContent><DropdownMenuItem onSelect={(event) => { event.stopPropagation(); onEditTask(task); }}>编辑任务</DropdownMenuItem><DropdownMenuItem onSelect={(event) => { event.stopPropagation(); onArchiveTask(task); }}>归档任务</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem className="dangerMenuItem" onSelect={(event) => { event.stopPropagation(); onDeleteTask(task); }}>删除任务</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>
