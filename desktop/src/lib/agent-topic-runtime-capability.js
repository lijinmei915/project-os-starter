export function runtimeCapabilitySpecs(topicId, { activeTaskCount, currentTask, failedTaskCount, memoryCount, profileMissingCount, recentResultCount, snapshot, taskNextActionLabel, visibleTaskCount }) {
  ...
  const memory = {
    "project-facts": { files: [".project-os/project-profile.json", ".project-os/workspace-facts.json", ".project-os/state.json", "PROJECT.md"], next: profileMissingCount ? "补齐缺失事实，并标记哪些来自用户确认、哪些来自文件推断。" : "把事实来源、可信度和更新时机展示出来。", status: profileMissingCount ? "待补齐" : "已识别", title: "项目事实", tone: profileMissingCount ? "warning" : "success", value: "让接入项目不再从零理解，Agent 可以直接知道项目定位、阶段、技术栈和治理域。" },
    "user-preferences": { files: [".project-os/project-profile.json", "OmniDesk global: user-profile.json"], next: "提供偏好确认入口，并区分项目级偏好和全局偏好。", status: snapshot?.projectProfile?.userPreferences ? "已记录" : "待确认", title: "用户偏好", tone: snapshot?.projectProfile?.userPreferences ? "success" : "warning", value: "让 Agent 记住用户对协作方式、视觉偏好、风险边界和表达方式的要求。" },
    "long-term-memory": { files: [".project-os/memory/*", "docs/LESSONS.md", "docs/DECISIONS.md", "HANDOFF.md"], next: "把重要对话、决策和踩坑经验沉淀为可复用长期记忆。", status: memoryCount ? "已接入" : "待沉淀", title: "长期记忆", tone: memoryCount ? "success" : "warning", value: "让接入项目保留长期上下文，避免重复解释历史决策和约束。" },
    "conversation-summary": { files: [".project-os/conversations/*", ".project-os/runs/desktop-tasks/*"], next: "把会话摘要和任务、项目事实、偏好更新关联起来。", status: "待产品化", title: "会话摘要", tone: "warning", value: "让对话不只是聊天记录，而能沉淀成任务、事实、偏好和交接线索。" },
  };
