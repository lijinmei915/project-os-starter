...
pub fn is_question_like_message(message: &str) -> bool {
    ...
    [
        "为什么", "怎么", "哪些", "还有哪些", "是什么", "吗", "呢", "咋回事", "看一下",
        "看看", "风险", "问题在哪", "自然吗", "正常吗", "why", "how", "what", "which",
        "risk", "risks",
    ]
    .iter()
...
pub fn is_task_like_message(message: &str) -> bool {
    ...
    [
        ...
        "帮我执行", "帮我跑", "开始执行", "生成计划", "创建任务", "改代码", "修复", "实现",
        "接入", "配置", "做成", "设计", "重构", "提交", "推送", "帮我处理", "处理一下",
        "解决一下", "看看解决", "看下解决", "整理一下", "梳理一下", "制定方案", "出个方案",
        "给个方案", "整理待办", "处理方案", "commit", "push", "build", "apply patch",
    ]
    .iter()
    .any(|keyword| text.contains(keyword))
}
...
mod tests {
    ...
    fn task_intent_overrides_question_words() {
        assert!(should_create_plan_for_message("帮我处理一下这个风险", false));
